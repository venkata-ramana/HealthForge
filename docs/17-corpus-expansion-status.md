...
The retrieval evaluation baseline remains intentionally failing for exact expected-source recall. That result is retained because the current retrieval configuration and coarse HTML document chunking do not yet satisfy the broader 26-case dataset, even after expanding to six sources. Package downloads remain a separate controlled path because they require package integrity and dependency validation.
